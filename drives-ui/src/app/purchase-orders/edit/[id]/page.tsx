...
export default function PurchaseOrderEdit() {
  ...
  const [displayPricingPremium, setDisplayPricingPremium] = useState<number>(0);
  ...
  const { selectProps: colorSelectProps } = useSelect({ resource: "colors" });

  // Watch form fields
  const purchaseTypeField = Form.useWatch("purchaseType", form) || "";
  const metalPriceValue = Form.useWatch("metalPriceValue", form) || 0;
  const pricingPremiumRaw = Form.useWatch("pricingPremium", form) || 0;
  ...
  const getGramsEquivalent = (unitId: number | string) => {
    if (!unitId || !quantityUnits.length) return 1;
    const unit = quantityUnits.find((u: any) => u.id === unitId);
    return unit?.gramsEquivalent || 1;
  };

  // Function to normalize quantity to grams
  const normalizeToGrams = (quantityValue: number, unitId: number | string) => {
  ...
  // Function to calculate total for a single line
  ...
  // Function to calculate and update line pricePerGram values
  const updateLinePricePerGram = () => {
    if (!lines.length || !materials.length || !metalPriceValue) return;

    // Use displayPricingPremium (percentage) divided by 100 to get decimal
    const effectivePrice =
      metalPriceValue + metalPriceValue * (displayPricingPremium / 100);
    const basePricePerGram =
      effectivePrice / normalizeToGrams(1, metalPriceUnitId);

    const newLines = lines.map((line: any) => {
      if (!line) return line;

      // If line already has a pricePerGram from the API, keep it
      if (line.pricePerGram && line.pricePerGram > 0) {
        return line;
      }

      const material = materials.find((m: any) => m.id === line.materialId);
      const purity = material?.purity ? Number(material.purity) : 1;
      const newPricePerGram = basePricePerGram * purity;
  ...
  // Handler for onCalculationUpdate from PricingCalculator
  ...
  // Effect to initialize form values after data loads
  useEffect(() => {
    if (!hasInitialized.current && formProps.initialValues) {
      console.log("Initial values:", formProps.initialValues);

      // Convert pricingPremium from decimal to percentage for display
      if (formProps.initialValues.pricingPremium !== undefined) {
        const premiumPercentage = formProps.initialValues.pricingPremium * 100;
        console.log(
          "Converting premium from",
          formProps.initialValues.pricingPremium,
          "to",
          premiumPercentage,
        );

        // Set the form field to percentage value
        form.setFieldValue("pricingPremium", premiumPercentage);
        // Also update our state
        setDisplayPricingPremium(premiumPercentage);
      }
  ...
  // Effect to sync pricingPremium between form and display state
  useEffect(() => {
    // pricingPremiumRaw is the raw form value (could be decimal or percentage depending on initialization)
    // We need to ensure we're working with percentage for calculations
    if (pricingPremiumRaw !== undefined) {
      // If the value is less than 1, it's likely still in decimal format (not initialized yet)
      // Once initialized, it should be >= 1 for percentages
      if (pricingPremiumRaw < 1 && hasInitialized.current) {
        // This shouldn't happen after initialization, but just in case
        const percentageValue = pricingPremiumRaw * 100;
        setDisplayPricingPremium(percentageValue);
      } else {
        // Value is already in percentage format
        setDisplayPricingPremium(pricingPremiumRaw);
      }
    }
  }, [pricingPremiumRaw]);

  // Effect to update line prices when metal price or premium changes
  useEffect(() => {
    if (metalPriceValue && materials.length && lines.length) {
      updateLinePricePerGram();
    }
  }, [
    metalPriceValue,
    displayPricingPremium,
    materials,
    lines.length,
    normalizeToGrams(1, metalPriceUnitId),
  ]);

  // Also trigger calculation when materials load
  useEffect(() => {
    if (metalPriceValue && materials.length && lines.length) {
      updateLinePricePerGram();
    }
    ...
    <Edit saveButtonProps={saveButtonProps}>
      <Form
        ...
        onFinish={(values) => {
          ...
          // Clone values to avoid mutation issues
          const processedValues = { ...values };

          // Normalize pricingPremium to decimal (e.g., 5 → 0.05) if it's a percentage
          if (
            "pricingPremium" in processedValues &&
            typeof processedValues.pricingPremium === "number"
          ) {
            processedValues.pricingPremium /= 100;
          }

          // Format purchaseDate to "YYYY-MM-DD" if it's a Dayjs object
          if (
            "purchaseDate" in processedValues &&
            dayjs.isDayjs(processedValues.purchaseDate)
          ) {
            processedValues.purchaseDate =
              processedValues.purchaseDate.format("YYYY-MM-DD");
          }
      ...
      >
          ...
          <Row gutter={[16, 24]}>
            <Col span={4}>
              ...
              <Form.Item
                ...
                rules={[{ required: true }]}
                getValueProps={(value) => {
                  // Convert string to Dayjs object for DatePicker
          ...
          <Row gutter={[16, 24]}>
            <Col span={12}>
              <PricingCalculator
                ...
                pricingPremium={displayPricingPremium / 100} // Convert percentage to decimal for calculator
        ...
        <Card title="Order Lines" style={{ marginTop: "24px" }}>
